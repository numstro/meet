...
export default function CreateDemoPollPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasEmptyBuckets) {
      alert('Please select at least one time of day for each date')
    ...
    setTimeout(() => {
      ...
      // Redirect to success page with poll details
      const pollTitle = encodeURIComponent(pollData.title || 'Your Poll')
      router.push(`/poll-created?title=${pollTitle}&id=demo`)
    }, 1000)
  }
    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            {isSubmitting ? 'Creating...' : 'Create Poll (Demo)'}
          </button>
        </div>
      </form>
    </div>
